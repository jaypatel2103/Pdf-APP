...
<BodyManagerWithPreview>
    ...
    IconButton:
        mipmap: True
        source: root.path
        on_release: root.events_callback(os.path.join(root.realpath , root.name))
    
    LabelContent:
        text: root.name

<BodyManagerWithPreviewForFiles>
    name: ""
    a: False
    path: ""
    realpath: ""
    ...
    CheckBox:
        ...
        on_active: root.events_callback(root.path,self.active)
        
    IconButton:
        mipmap: True
        source: os.path.join(root.realpath, root.name)
        on_release: chebox.active = True if root.a == True else chebox.active is False
...
class BodyManagerWithPreviewForFiles(MDBoxLayout):
    """Base class for folder icons and thumbnails images in ``preview`` mode.
    """
    # def clear_on_next_dir(self):
    #     self.ids.chebox.active = False
...
class MDFileManager(ThemableBehavior, MDFloatLayout):
        ...
        if self.preview:
            ...
            for name_file in files:
                ...
                if os.path.splitext(name_file)[1] in self.ext:
                    manager_list.append(
                        {
                            "viewclass": "BodyManagerWithPreviewForFiles",
                            "path": name_file,
                            "name": file_name,
                            "realpath": os.path.join(path),
                            "type": "files",
                            "events_callback": self.select_file,
                            "height": dp(150),
                        }
                    )
        else:
    ...
    def select_dir_or_file(self, path):
        ...
        self.current_path = path
        # BodyManagerWithPreviewForFiles.clear_on_next_dir(BodyManagerWithPreviewForFiles())
        self.list_of_selected_images_or_pdfs = []
        self.show(path)

    def select_file(self, path, checkbox):
        """Called by tap on the name of the directory or file."""

        if checkbox is True:
            self.list_of_selected_images_or_pdfs.append(path)
        else:
            try:
                self.list_of_selected_images_or_pdfs.remove(path)
            except:
                print('error')
    ...
    def select_directory_on_press_button(self, *args):
        ...
        self.exit_manager(self.list_of_selected_images_or_pdfs,"selection_done",)
